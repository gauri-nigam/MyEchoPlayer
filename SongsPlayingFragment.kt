...
import kotlinx.android.synthetic.main.fragment_songs_playing.view.*
import java.util.*
...
class SongsPlayingFragment : Fragment() {
    ...
    object Staticated{
            ...
            if (Statified.currentSongHelper?.isShuffle as Boolean) {
               playNext("PlayNextLikeNormalShuffle")
                ...
                if (Statified.currentSongHelper?.isLoop as Boolean) {
                    Statified.currentSongHelper?.isPlaying = true
                    var nextSong = Statified.fetchSongs?.get(Statified.currentPositon)

                    Statified.currentSongHelper?.songTitle = nextSong?.songTitle
                    Statified.currentSongHelper?.songArtist = nextSong?.songArtist
                    Statified.currentSongHelper?.currentPosition = Statified.currentPositon
        ...
        fun playNext(check: String) {
            if (check.equals("PlayNextNormal", true)) {
                Statified.currentPositon = Statified.currentPositon + 1
            } else if (check.equals("PlayNextLikeNormalShuffle", true)) {
                var randomObject = Random()
                var randomPosition = randomObject.nextInt(Statified.fetchSongs?.size?.plus(1) as Int)
                Statified.currentPositon = randomPosition
            }
            if (Statified.currentPositon == Statified.fetchSongs?.size) {
                Statified.currentPositon = 0
            }
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?,
        ...
        Statified.fab?.alpha= 0.8f// to dim the heart icon
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            Statified.currentPositon = arguments.getInt("songPosition")
...
//
    ...
    fun clickHandler() {
        ...
        Statified.fab?.setOnClickListener({
            if(Statified.favoriteContent?.checkifIdExists(Statified.currentSongHelper?.songId?.toInt() as Int)as Boolean){
                ...
                Statified.fab?.setImageDrawable(ContextCompat.getDrawable(Statified.myActivity, R.drawable.favorite_on))
                Statified.favoriteContent?.storeAsFavourite(Statified.currentSongHelper?.songId?.toInt()as Int, Statified.currentSongHelper?.songArtist,
    ...
    fun playPrevious() {
        ...
        Statified.currentSongHelper?.isLoop = false
        val nextSong = Statified.fetchSongs?.get(Statified.currentPositon)
        Statified.currentSongHelper?.songTitle = nextSong?.songTitle
        Statified.currentSongHelper?.songArtist = nextSong?.songArtist
        Statified.currentSongHelper?.currentPosition =Statified. currentPositon
        Statified.currentSongHelper?.songId = nextSong?.songID as Long
